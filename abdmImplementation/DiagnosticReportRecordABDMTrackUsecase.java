...
//	 Populate "Observation/LipidPanel" Resource
	public static Observation populateLipidPanelResource() {
		...
		observation.addComponent().setCode(new CodeableConcept(
				new Coding("http://loinc.org", "2093-3", "Cholesterol [Mass/volume] in Serum or Plasma"))).setValue(
						new Quantity().setValue(162).setUnit("mg/dl").setSystem("http://unitsofmeasure.org").setCode("mg/dl"));

		observation.addComponent().setCode(new CodeableConcept(
				new Coding("http://loinc.org", "2085-9", "Cholesterol [Mass/volume] in Serum or Plasma"))).setValue(
				new Quantity().setValue(43).setUnit("mg/dl").setSystem("http://unitsofmeasure.org").setCode("mg/dl"));

		observation.addComponent().setCode(new CodeableConcept(
				new Coding("http://loinc.org", "13457-7", "Cholesterol [Mass/volume] in Serum or Plasma"))).setValue(
				new Quantity().setValue(84).setUnit("mg/dl").setSystem("http://unitsofmeasure.org").setCode("mg/dl"));

		observation.addComponent().setCode(new CodeableConcept(
				new Coding("http://loinc.org", "2571-8", "Cholesterol [Mass/volume] in Serum or Plasma"))).setValue(
				new Quantity().setValue(177).setUnit("mg/dl").setSystem("http://unitsofmeasure.org").setCode("mg/dl"));

		observation.addComponent().setCode(new CodeableConcept(
				new Coding("http://loinc.org", "9830-1", "Cholesterol [Mass/volume] in Serum or Plasma"))).setValue(new Quantity().setValue(177).setUnit("{ratio}"));
		return observation;
...
//
	...
	static void init() throws DataFormatException, FileNotFoundException
	{
		...
		String[] fileList = new File("F:\\NDHMProfiles\\").list(new WildcardFileFilter("*.xml"));
		for(String file:fileList)
		{
			//Parse All Profiles and add to prepopulated support
			sd = parser.parseResource(StructureDefinition.class, new FileReader("F:\\NDHMProfiles\\"+file));
